...
including the concurrency work in 1.1.0, is mine. This document is the agent's
...
drifting. In 1.1.0 there is one code path and the test is a regression guard,
...
(Both details changed in 1.1.0, for reasons that have nothing to do with Lumo:
...
Written after 1.1.0, which is the first version reviewed as a public package
rather than as an extraction. Everything below was already true in Lumo. The
extraction preserved it faithfully, which is exactly what an extraction should
do, and is also why an extraction is not a review.
